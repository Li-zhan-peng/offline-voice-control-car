...
void Application::Start() {
    ...
    // OFFLINE MODE: if there is no network, jump straight to idle state so that
    // local (offline) voice commands keep working without WiFi or cloud.
    if (!WifiStation::GetInstance().IsConnected()) {
        ESP_LOGW(TAG, "=== OFFLINE MODE: no network, local voice commands only ===");
        SetDeviceState(kDeviceStateIdle);
