...
void WifiBoard::StartNetwork() {
    ...
    if (ssid_list.empty()) {
        ...
        // OFFLINE MODE: do NOT block in config AP. Return immediately so the
        // application can continue to idle state and enable local commands.
        ESP_LOGW(TAG, "No saved WiFi -> OFFLINE mode (skip blocking config AP)");
        return;
    }
    ...
    // Try to connect to WiFi; if failed, continue in OFFLINE mode (do not block)
